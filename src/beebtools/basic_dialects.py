...
def _toKeyword(spec: TokenSpec) -> Keyword:
    ...
    return Keyword(name=spec.name, token=spec.byte, flags=spec.flags)
...
def _buildKeywordRows(specs: Tuple[TokenSpec, ...]) -> Tuple[Keyword, ...]:
    ...
    linear matcher walks, so COMMON_ABBREV rows (PRINT before PAGE,
    ...
    rows.sort(key=lambda kw: (
        Flag.COMMON_ABBREV not in kw.flags, -len(kw.name), kw.token
    ))
