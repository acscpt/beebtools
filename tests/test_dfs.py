...
Tests are parametrized over every .ssd and .dsd file found in
tests/resources/discs/ and its subdirectories (including the synthetic
images generated by tests/scripts/create_test_images.py). The suite is
skipped automatically when no images are present.
...
# Discover all DFS disc images in the resources directory tree.
DISCS_DIR = os.path.join(os.path.dirname(__file__), "resources", "discs")
ALL_DFS = sorted(
    glob.glob(os.path.join(DISCS_DIR, "**", "*.ssd"), recursive=True)
    + glob.glob(os.path.join(DISCS_DIR, "**", "*.dsd"), recursive=True)
)

# Readable test IDs: just the filename, not the full path.
disc_ids = [os.path.basename(p) for p in ALL_DFS]

# Skip the entire module when no discs are present (e.g. in CI).
pytestmark = pytest.mark.skipif(
    len(ALL_DFS) == 0,
    reason="No DFS disc images found in tests/resources/discs/",
)
...
class TestCatalogueStructure:
    ...
    @pytest.mark.parametrize("path", ALL_DFS, ids=disc_ids)
    def testSideCountMatchesFormat(self, path):
        """An SSD image should have one side, a DSD image should have two."""
        image = openDiscImage(path)
